...
import { Cell, SendMode, storeMessageRelaxed } from '@ton/core';
...
const MNEMONIC = process.env.CONTRACT_MNEMONIC;
const SUBWALLET_ID = process.env.CONTRACT_SUBWALLET_ID;
const TIMEOUT = process.env.CONTRACT_TIMEOUT;
const ENDPOINT = process.env.CONTRACT_ENDPOINT;
const API_KEY = process.env.API_KEY_TG;
const CODE_HEX = process.env.CODE_HEX;
...
// Проверка
if (!MNEMONIC) {
    console.error('❌ MNEMONIC is undefined in tonWithdraw.js');
}

// Инициализация CODE
let CODE;
if (CODE_HEX) {
    CODE = Cell.fromBoc(Buffer.from(CODE_HEX, 'hex'))[0];
}
