...
<transition name="filter-dropdown">
  ...
  >
   ...
   <div class="balance-filter-container" v-if="activeTab === 'market'">
      <div  class="ton-balance balance-container">
...
<div v-if="activeTab === 'activity'" class="activity-page">
  <div class="activity-container">
    ...
    <div class="filter-buttons">
      ...
      <div class="my-activity-toggle">
        <label class="toggle-label">
          <span class="toggle-text">My Activity</span>
          <div class="toggle-switch" :class="{ active: showOnlyMyActivity }" @click="toggleMyActivity">
    ...
    <div class="activity-list">
      <div v-if="filteredActivity.length === 0" class="empty-activity">
      ...
      <div v-else class="activity-items">
        <div 
          ...
          :key="activity.id"
          class="activity-item"
          :class="[activity.operation_type, { 'my-activity': activity.user_name === currentAccountName }]"
        >
          <!-- NFT Preview -->
          <!-- В template секции, найдите блок activity-nft-preview и замените его: -->
<div class="activity-nft-preview">
  <div v-if="activity.html_content" class="cart-item-html-wrapper">
    <iframe 
      sandbox="allow-scripts allow-same-origin"
      :srcdoc="getActivityNftHtml(activity)"
      class="cart-item-html-content"
    ></iframe>
  </div>
  <img v-else :src="getNftImage(activity.nft_type)" :alt="activity.nft_type" class="cart-item-image">
</div>
              ...
              <div class="activity-info-left">
                <div class="activity-header-info">
                  <h4 class="activity-nft-name">
                    ...
                    <span v-if="activity.nft_object" class="nft-id">
...
<div v-if="activeTab === 'my-nfts'" class="market-page">
      ...
      <template v-if="myNftsViewMode === 'listed'">
        <div v-for="item in filteredMarketItems" :key="item.id" class="market-item" :class="{ 'ton': item.currency === 'TON' }">
          ...
          <div v-if="item.html_content" class="nft-html-wrapper">
            <iframe 
    sandbox="allow-scripts allow-same-origin"
    :srcdoc="generateNftHtmlForList(item.nft_object || item)"
  ...
  @click.stop="openFullscreenView(item)"
>
  Cancel sale
</button>
          </div>
      ...
      <template v-else>
        ...
        <div v-for="item in filteredMarketItems" :key="'unlisted-'+item.id" class="market-item">
          ...
          <div class="nft-html-wrapper">
            <iframe 
              sandbox="allow-scripts allow-same-origin"
              :srcdoc="generateNftHtmlForFullscreen(item.nft_object)"
              class="nft-html-content"
            ></iframe>
          </div>
...
<!-- В шаблоне, в секции market-item -->
<div v-for="item in filteredMarketItems" :key="item.id" class="market-item" :class="{ 'ton': item.currency === 'TON' }">
    <!-- HTML контент с иконкой корзины поверх -->
    <div v-if="item.html_content" class="nft-html-wrapper">
      <div class="nft-html-container">
        <iframe 
          sandbox="allow-scripts allow-same-origin"
          :srcdoc="generateNftHtmlForList(item.nft_object || item)"
          class="nft-html-content"
        ...
        <!-- Иконка корзины поверх HTML объекта -->
        <button 
...
<div class="html-container">
  ...
  :disabled="htmlModal.isUnlisted || (htmlModal.item && htmlModal.item.seller === currentAccountName)"
  :title="htmlModal.isUnlisted ? 'Not available for unlisted items' : (htmlModal.item && isInCart(htmlModal.item.id) ? 'Remove from cart' : 'Add to cart')"
  :class="{ 
    'in-cart': htmlModal.item && isInCart(htmlModal.item.id),
    'disabled': htmlModal.isUnlisted
  }"
>
  <img 
  ...
  title="Button 1"
  :disabled="htmlModal.isUnlisted"
  :class="{ 'disabled': htmlModal.isUnlisted }"
>
  <img :src="share" alt="Button 1" class="action-button-icon">
</button>
    <button class="action-button-row" title="Button 3">
      <img :src="giftbox" alt="Button 3" class="action-button-icon">
    </button>
  </div>
...
<div class="action-buttons">
  ...
  <div class="unlisted-actions-fullwidth">
    <!-- Кнопка Send (желтая) -->
    <button 
      v-if="htmlModal.actionType === 'send'"
      class="send-button-fullscreen full-width-button" 
    ...
    <button 
      ...
      class="withdraw-button-fullscreen full-width-button" 
    ...
    <button 
      v-else
      class="sell-button-fullscreen full-width-button" 
...
<div class="cart-item-preview">
  ...
  <div class="cart-item-info">
    ...
    <div class="modal-dropdown-actions">
      ...
      <button @click="executeCancelSale" class="confirm-button">Confirm</button>
    </div>
  </div>
</div>
  ...
  <div class="cart-item-info">
    ...
    <div class="modal-dropdown-actions">
      ...
      <button 
        ...
        class="confirm-button"
        :disabled="!editPriceModal.newPrice || editPriceModal.newPrice <= 0"
      >
        Обновить цену
      </button>
    </div>
...
<div v-if="sellModal.show" class="modal-dropdown-overlay" @click="sellModal.show = false">
  <div class="modal-dropdown" @click.stop>
    ...
    <div class="modal-dropdown-header">
      <h3 class="modal-dropdown-title">Выставление на продажу</h3>
    </div>
    
    <div class="modal-dropdown-content">
      <!-- NFT Preview -->
      <div class="cart-item-preview">
  <div v-if="sellModal.nftData" class="cart-item-html-wrapper">
    <iframe 
      sandbox="allow-scripts allow-same-origin"
      :srcdoc="generateNftHtmlForFullscreen(sellModal.nftData)"
      class="cart-item-html-content"
    ></iframe>
  </div>
  <img v-else :src="getNftImage(sellModal.nftType)" :alt="sellModal.nftType" class="cart-item-image">
  
  <div class="cart-item-info">
    <div class="cart-item-name-container">
      <span class="cart-item-name">{{ extractNftDataWithNumber(sellModal.nftData).name }}</span>
      <span v-if="extractNftDataWithNumber(sellModal.nftData).number" class="cart-item-number">
    ...
    <div class="modal-dropdown-actions">
      ...
      <div v-else>
        ...
        <!-- В модальном окне продажи найдите кнопку подтверждения -->
...
<div v-if="transferModal.show" class="modal-dropdown-overlay" @click="transferModal.show = false">
  ...
  <div v-if="transferModal.nftData" class="cart-item-html-wrapper">
    <iframe 
      sandbox="allow-scripts allow-same-origin"
      :srcdoc="generateNftHtmlForFullscreen(transferModal.nftData)"
      class="cart-item-html-content"
    ></iframe>
  </div>
  ...
  <div class="cart-item-info">
    ...
    <!-- Добавьте этот блок в template после других модальных окон -->
<!-- Убедитесь, что этот блок есть в template -->
<transition name="fade">
  <div v-if="operationSuccess.show" class="purchase-success-overlay">
    <div class="purchase-success-content">
      <div 
        ref="lottieContainer" 
        class="lottie-container"
      ></div>
      <h3 class="success-title">{{ operationSuccess.title }}</h3>
    </div>
  </div>
</transition>
...
import purschaseIcon from '@/assets/purschase.png';
...
// Упрощенная функция открытия полноэкранного просмотра
...
// Также убедитесь, что эта функция существует:
  ...
  try {
    ...
    for (const item of cartItems.value) {
      ...
      if (sellerData) {
        ...
        const itemCost = item.price_per_unit * (item.amount || 1);
        const newSellerBalance = sellerCurrentBalance + itemCost;
...
const getCurrentDataSource = computed(() => {
  if (activeTab.value === 'market') {
    ...
    // Преобразуем activity items в формат похожий на market items для работы фильтров
    return activityItems.value.map(activity => {
      let nftData = {};
      ...
      try {
        ...
        console.error('❌ Ошибка парсинга nft_object в activity:', parseError);
      }
      
      return {
        id: `activity-${activity.id}`,
        nft_type: activity.nft_type || `html_nft_${nftData.name?.replace(/\s+/g, '') || 'unknown'}`,
        nft_object: nftData,
  ...
  try {
    ...
    if (error) {
      ...
      if (data && data.length > 0) {
        console.log('📋 Пример записи активности:', data[0]);
      } else {
...
const addActivityRecord = async (activityData) => {
  try {
    ...
    const record = {
      ...
      price_per_unit: activityData.price_per_unit || null,
    ...
    // Автоматически обновляем список если мы на вкладке Activity
...
// Функция для переключения "My Activity"
const toggleMyActivity = () => {
  showOnlyMyActivity.value = !showOnlyMyActivity.value;
  console.log(`🔍 My Activity: ${showOnlyMyActivity.value ? 'ON' : 'OFF'}`);
};
...
// Функция выполнения изменения цены
...
const extractBackdropFromNft = (item) => {
  try {
    ...
    if (item.backdrop) {
      ...
      console.log('📋 Backdrop из поля backdrop:', backdrop);
    }
...
const logAllMarketSymbols = () => {
  console.log('=== ССЫЛКИ НА СИМВОЛЫ С МАРКЕТА ===');
  ...
  console.log(`📊 Всего ссылок: ${allSymbols.size}`);
  console.log('==================================');
};
    ...
    return new Promise((resolve, reject) => {
      ...
      const messageHandler = (event) => {
        if (event.data.type === 'symbolImageReady' && event.data.symbolName === symbolName) {
          console.log(`✅ TGS сконвертирован в изображение для: ${symbolName}`);
...
// Добавьте forceUpdate для принудительного обновления
...
const extractNameFromNftObject = (item) => {
  try {
    ...
    // Если есть прямое имя в nft_object
    if (item.nft_object && item.nft_object.name) {
      name = item.nft_object.name;
    }
    // Если это unlisted NFT с данными из nft_links
    else if (item.is_unlisted && item.nft_object && item.nft_object.name) {
      name = item.nft_object.name;
    }
    // Используем стандартную функцию как fallback
    else {
      name = getNftDisplayName(item.nft_type, item.nft_object);
    }
    
    // Добавляем номер если есть
    const nftNumber = extractNftNumberFromItem(item);
    if (nftNumber) {
      return {
        name: name,
        number: nftNumber,
        fullName: `${name} #${nftNumber}`
      };
    }
    ...
    const fallbackName = getNftDisplayName(item.nft_type);
    return {
      name: fallbackName,
      number: null,
...
// Функция для генерации HTML контента для символа
...
// Обновите функцию getFilteredSymbols для использования HTML контента
const getFilteredSymbols = () => {
  const query = searchQuery.value ? searchQuery.value.toLowerCase() : '';
  
  console.log('🔍 Поиск символов, запрос:', query);
  console.log('📊 Доступные символы:', availableSymbolOptions.value);
  console.log('🖼️ HTML кэш:', symbolHtmlCache.value);
  ...
  if (!availableSymbolOptions.value || availableSymbolOptions.value.length === 0) {
    console.log('❌ Нет доступных символов на маркете');
    return [];
  }
  
  const filtered = availableSymbolOptions.value
    .filter(symbol => {
    ...
    .map(symbol => {
      // Используем HTML контент из кэша
      const htmlContent = symbolHtmlCache.value[symbol.name] || null;
      
      const result = {
        name: symbol.name,
        value: normalizeSymbolName(symbol.name),
        htmlContent: htmlContent,
        floorPrice: getFloorPrice('symbol', symbol.name),
      ...
      if (result.percentage < 1) {
        result.rarityClass = 'ultra-rare';
      } else if (result.percentage < 5) {
        result.rarityClass = 'rare';
      } else if (result.percentage < 15) {
        result.rarityClass = 'uncommon';
      } else {
        result.rarityClass = 'common';
      }
      
      console.log(`✅ Символ "${symbol.name}": редкость=${result.rarityText}, HTML=${!!result.htmlContent}`);
...
const extractSymbolWithPercentage = (item) => {
  try {
    console.log('🔍 Извлечение символа с редкостью для item:', item.id);
    ...
    console.log('✅ Извлеченный символ:', { name: symbolName, percentage });
...
const availableSymbolOptions = computed(() => {
  ...
  preFilteredItems.forEach(item => {
    ...
    if (symbolData.name) {
      ...
      // Сохраняем самую низкую редкость (самый редкий вариант)
...
// Функции getSelectedSymbolOptions и getUnselectedSymbolOptions остаются без изменений
...
const initiateBuy = (item) => {
  if (!item || !item.id) {
    showNotification('Ошибка: некорректные данные товара', 'error');
    return;
  }
...
const openEditPriceModal = (item) => {
  editPriceModal.value = {
    ...
    item: item,
    nftType: item.nft_type,
    currentPrice: item.price_per_unit, // ← Важно!
...
const handleSell = async (nftData) => {
  try {
    ...
    sellModal.value = {
      show: true,
      nftType: isHtmlNft ? `html_nft_${extractNftIdFromUnlisted(nftData).replace('#', '')}` : nftData.nft_type,
      amount: isHtmlNft ? 1 : '',
      price: '',
      confirmStep: false,
      isHtmlNft: isHtmlNft,
...
// Упрощенная executeSell (теперь проверки уже сделаны)
// Исправленная функция executeSell
const executeSell = async () => {
  try {
    const { nftType, amount, price, isHtmlNft, nftData } = sellModal.value;
    ...
    if (isHtmlNft) {
      ...
      const htmlContent = generateNftHtmlForList(nftData);
      ...
      const { data: marketItem, error: marketError } = await supabase
        ...
        .insert([{
          ...
          total_price: price, // total_price = price * amount (amount = 1)
          is_html_nft: true,
          html_content: htmlContent, // Используем сгенерированный контент
    ...
    const availableAmount = userNfts[nftType] || 0;
    ...
    const { data: marketItem, error: marketError } = await supabase
      ...
      .insert([{
        ...
        total_price: numPrice * numAmount,
        currency: 'TON'
      }]);

    if (marketError) throw marketError;

    // Записываем активность Listing для обычных NFT
    await recordListingActivity({
      seller: currentAccountName.value,
      item_id: itemId,
      nft_type: nftType,
    ...
    // Обновляем количество NFT у пользователя
    userNfts[nftType] = availableAmount - numAmount;
    
    const { error: updateError } = await supabase
      .from('users')
      .update({ nfts: userNfts })
      .eq('name', currentAccountName.value);
    ...
    // 1. СНАЧАЛА закрываем модальное окно продажи
    sellModal.value.show = false;
    sellModal.value.confirmStep = false;

    // 2. Ждём следующего тика DOM чтобы модалка успела закрыться
    await nextTick();
    
    // 3. ПОТОМ показываем анимацию успеха
    console.log('🎯 Показываем анимацию listing');
...
// Функция для форматирования цены (убирает лишние нули)
const formatPrice = (price) => {
  ...
  // Если дробное - показываем до 2 знаков, убирая лишние нули
  ...
  console.log('Add NFT clicked');
    ...
    const executeTransfer = async () => {
      try {
        ...
        await recordTransferActivity(nftData, recipient, amount);


        showNotification('NFT успешно передано!', 'success');
        transferModal.value.show = false;
        transferModal.value.confirmStep = false;

        // Показываем успешную передачу с анимацией
...
// Функция для определения какой HTML использовать для активности
const getActivityNftHtml = (activity) => {
  try {
    ...
    if (isOnMarket) {
      ...
      return generateNftHtmlForList(activity.nft_object || activity);
    } else {
      // Если объявления нет на рынке - используем полноэкранный вид
      return generateNftHtmlForFullscreen(activity.nft_object || activity);
    }
  } catch (error) {
...
// Функция для проверки наличия объявления на рынке
// Оптимизированная версия функции проверки
const checkIfActivityItemIsOnMarket = (activity) => {
  try {
    // Быстрая проверка по item_id
    if (activity.item_id && marketItemsMap.value[activity.item_id]) {
      return true;
    }
    
    // Более глубокая проверка для случаев когда item_id нет или не совпадает
    const marketItem = marketItems.value.find(item => {
      // Сравниваем по различным идентификаторам
      if (activity.item_id && item.item_id === activity.item_id) return true;
      
      // Сравниваем по NFT данным
      const activityNftId = extractNftIdFromData(activity.nft_object || activity);
      const itemNftId = extractNftIdFromData(item.nft_object || item);
      
      if (activityNftId !== 'unknown' && activityNftId === itemNftId) {
        return true;
      }
      
      // Сравниваем по названиям
      const activityName = getNftDisplayName(activity.nft_type, activity.nft_object);
      const itemName = getNftDisplayName(item.nft_type, item.nft_object);
      
      return activityName === itemName;
    });
...
// Добавьте watcher для вкладки активности
watch(() => activeTab.value, (newTab) => {
  if (newTab === 'activity' && currentAccountName.value) {
    loadActivityHistory();
  }
});
...
// Функция для записи операции выставления на продажу (Listing)
const recordListingActivity = async (item) => {
  try {
    if (!currentAccountName.value) {
      console.log('👤 Пользователь не авторизован, запись Listing не добавлена');
      return null;
    }

    const record = {
      user_name: item.seller || currentAccountName.value,
      item_id: item.item_id || item.id,
      nft_type: item.nft_type,
      nft_object: item.nft_object,
      html_content: item.html_content,
      ...
      currency: item.currency || 'TON',
      amount: item.amount || 1,
      total_price: item.total_price || item.price_per_unit * (item.amount || 1),
      created_at: new Date().toISOString()
    };
    ...
    if (error) {
      console.error('❌ Ошибка добавления записи Listing:', error);
      return null;
    }

    console.log('✅ Запись Listing добавлена:', data[0]);
...
const activityTypes = [
  ...
  { value: 'price_edit', label: 'Price Edits' }
    ...
    // Исправленная функция executeCancelSale
...
const initiateCancelSale = (item) => {
  if (!item || !item.id) {
    console.error('❌ Неверные данные item для снятия с продажи:', item);
    showNotification('Ошибка: неверные данные объявления', 'error');
    return;
  }
...
const executeBuy = async () => {
  try {
    ...
    const totalCost = item.price_per_unit * buyAmount;
    const currentBalance = parseFloat(userData.ton_balance) || 0;
    ...
    // Получаем текущие nft_links пользователя
    const { data: userLinksData, error: fetchLinksError } = await supabase
      .from('users')
      .select('nft_links')
      .eq('name', currentAccountName.value)
      .single();
    ...
    const currentLinks = userLinksData.nft_links || [];
    const updatedLinks = [...currentLinks, nftData];

    // Обновляем баланс покупателя (списываем средства)
    const newBuyerBalance = currentBalance - totalCost;
    ...
    if (sellerData) {
      const sellerCurrentBalance = parseFloat(sellerData.ton_balance) || 0;
      const newSellerBalance = sellerCurrentBalance + totalCost;
    ...
    // Обновляем количество или удаляем item с рынка
    if (item.amount > buyAmount) {
      const newAmount = item.amount - buyAmount;
      const { error: updateMarketError } = await supabase
        .from('market')
        .update({ 
          amount: newAmount,
          total_price: item.price_per_unit * newAmount
        })
        .eq('id', item.id);
...
onMounted(async () => {
    ...
    watch(() => props.tonConnectUI?.connected, (connected) => {
      if (connected) {
        fetchTonBalance(); // Баланс из блокчейна
...
const handleBuyClick = async (item) => {
  ...
  try {
    ...
    const uniqueItems = [];
    const seenItems = new Map(); // Используем Map для более сложных ключей
    
    data.forEach(item => {
      // Создаем комплексный ключ для проверки дубликатов
      const itemKey = createMarketItemKey(item);
      
      if (!seenItems.has(itemKey)) {
        seenItems.set(itemKey, item);
        uniqueItems.push(item);
      } else {
        console.warn('🚫 Найден дубликат объявления:', itemKey, item);
        // Автоматически помечаем для удаления старый дубликат
        markDuplicateForRemoval(item.id);
      }
    });
    ...
    // ВЫВОДИМ СИМВОЛЫ ПОСЛЕ ЗАГРУЗКИ ДАННЫХ
    logAllMarketSymbols();
    marketItems.value = uniqueItems;
    
    console.log(`✅ Загружено ${uniqueItems.length} уникальных объявлений (было ${data.length})`);
...
const closeHtmlModal = () => {
  ...
  // Возвращаем пользователя на основной маршрут /shop
...
watch(filteredMarketItems, (newItems) => {
  console.log('🔄 Фильтры обновлены, текущие символы на маркете:');
  ...
  console.log('🎯 Символы после фильтрации:', Array.from(currentSymbols));
}, { deep: true });
...
.modal-dropdown {
  position: fixed;
  bottom: 80px; /* Поднимаем над нижним меню */
  left: 50%;
  transform: translateX(-50%);
  width: 95%;
  max-width: 500px;
