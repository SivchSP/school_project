...
<div class="action-buttons">
  <!-- Для полученных pending офферов (независимо от состояния ползунка) -->
  <template v-if="htmlModal.item && 
    ...
    <div class="pending-offer-actions-horizontal">
      <button 
        class="reject-button-fullscreen horizontal-button" 
        @click="rejectOffer(htmlModal.item)"
        :disabled="offerActionLoading"
      >
        <span v-if="offerActionLoading">Processing...</span>
        <span v-else>Reject Offer</span>
      </button>
      <button 
        class="accept-button-fullscreen horizontal-button" 
        @click="acceptOffer(htmlModal.item)"
        :disabled="offerActionLoading"
      >
        <span v-if="offerActionLoading">Processing...</span>
        <span v-else>Accept Offer</span>
      </button>
    </div>
  </template>
  
  <!-- Для всех остальных случаев НЕ показываем никакие кнопки -->
  <template v-else>
    <!-- Пустой блок - никаких кнопок -->
  </template>
</div>
  </div>
...
<div class="my-activity-toggle">
  <label class="toggle-label-activity">
    ...
    <div v-if="filteredOffers.length === 0" class="empty-activity">
...
const toggleMyActivity = () => {
  ...
  console.log(`📊 Логика: ${showOnlyMyActivity.value ? 
    'seller_name = currentUser AND buyer_name != currentUser' : 
    'seller_name != currentUser AND buyer_name = currentUser'}`);
};
// Остальные функции
...
const processAcceptedOffer = async (offer) => {
  try {
    ...
    // Находим NFT в коллекции продавца
    const sellerNfts = sellerData.nft_links || [];
    const nftToTransfer = sellerNfts.find(nft => {
      // Логика поиска нужного NFT по данным из оффера
      const nftUrl = typeof nft === 'object' ? nft.url : nft;
      return nftUrl === offer.nft_url; // Предполагаем, что в оффере есть nft_url
    });

    if (!nftToTransfer) {
      throw new Error('NFT не найдено в коллекции продавца');
    }

    // Удаляем NFT у продавца
    const updatedSellerNfts = sellerNfts.filter(nft => {
      const nftUrl = typeof nft === 'object' ? nft.url : nft;
      return nftUrl !== offer.nft_url;
    });
    ...
    const updatedBuyerNfts = [...buyerNfts, nftToTransfer];

    // 2. Обновляем балансы
    const offerPrice = offer.offer_price || offer.price_per_unit;
    
    // Списание с покупателя (если не было предварительного холда)
    ...
    const newBuyerBalance = (parseFloat(buyerBalanceData.ton_balance) || 0) - offerPrice;

    // Зачисление продавцу (минус комиссия)
    ...
    const newSellerBalance = (parseFloat(sellerBalanceData.ton_balance) || 0) + sellerAmount;

    // 3. Выполняем все обновления в транзакции
    const updates = [
      // Обновляем NFT продавца
      supabase
        .from('users')
        .update({ nft_links: updatedSellerNfts })
        .eq('name', currentAccountName.value),
...
const getFilteredNftOptions = async () => {
  ...
  const optionsPromises = Array.from(nftTypesWithNames.entries()).map(async ([nftType, displayName]) => {
    // Для БД используем оригинальное displayName (например, "Jack-in-the-Box")
    const imageUrl = await generateNftImageUrl(displayName, displayName.toLowerCase());
    const floorPrice = getFloorPriceForOffer('nft', nftType);
    
    return {
      originalType: nftType,
      name: displayName, // Оригинальное имя для отображения и БД
      imageUrl: imageUrl, // URL генерируется с displayName.toLowerCase()
  ...
  // Дальше остальная логика без изменений...
...
// Функция для загрузки всех offers (вызывать в onMounted)
const loadAllOffers = async () => {
  try {
    console.log('📥 Загрузка всех offers...');
    ...
    console.log(`✅ Загружено ${allOffers.value.length} offers`);
...
const loadNftOptionsWithImages = async () => {
  try {
    ...
    // ФУНКЦИЯ: Получение изображения NFT
    const getNftImage = (nftData) => {
      try {
        if (!nftData) return default_nft_image
        
        const data = typeof nftData === 'string' ? JSON.parse(nftData) : nftData
        
        // Пытаемся определить тип NFT из URL для выбора правильного изображения
        if (data.url) {
          const match = data.url.match(/\/nft\/([^-]+)/)
          if (match && match[1]) {
            const nftType = match[1].toLowerCase()
            const imageMap = {
              'cowboy': cowboy_nft_image,
              'stalker': stalker_nft_image,
              'lame': lame_nft_image,
              'boss': boss_nft_image,
              'nft10': nft10,
              'nft7': nft7,
              ...
              'nft9': nft9,
              'nft6': nft6,
              'jackinthebox': cowboy_nft_image, // пример маппинга
              // добавьте другие маппинги по необходимости
            }
            
            return imageMap[nftType] || default_nft_image
          }
        }
...
// Функция загрузки офферов
const loadOffers = async () => {
  try {
    console.log('🔄 Загрузка офферов...');
    ...
    offers.value = data || [];
    console.log(`✅ Загружено ${offers.value.length} офферов:`, offers.value);
    
    // Отладка структуры данных
    if (offers.value.length > 0) {
      console.log('📊 Пример данных оффера:', offers.value[0]);
      console.log('📋 nft_data первого оффера:', offers.value[0].nft_data);
    }
...
// Улучшенное computed свойство для фильтрации
const filteredOffers = computed(() => {
  console.group('🔍 ФИЛЬТРАЦИЯ ОФФЕРОВ');
  console.log('📦 Всего офферов:', offers.value.length);
  console.log('🎯 Выбранные фильтры:', selectedFilters.value);
  console.log('📊 Тип оффера:', selectedOfferType.value);
  console.log('🆔 ID фильтр:', selectedIdFilter.value);
  console.log('👤 My Offers:', showOnlyMyActivity.value);
  console.log('👤 Текущий пользователь:', currentAccountName.value);

  if (!offers.value || offers.value.length === 0) {
    console.log('❌ Нет офферов для фильтрации');
    console.groupEnd();
    return [];
  }
    ...
    filtered = filtered.filter(offer => {
      try {
        ...
        const matches = selectedFilters.value.nft.some(selectedNft => {
          ...
          console.log(`   🔍 Сравнение: "${normalizedOfferNft}" включает "${normalizedSelected}" = ${result}`);
          return result;
        });
  ...
  console.log(`✅ Результат фильтрации: ${filtered.length} офферов`);
