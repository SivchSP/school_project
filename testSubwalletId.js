import { TonClient } from '@ton/ton';
import { mnemonicToPrivateKey } from '@ton/crypto';
import { Cell } from '@ton/core';
import { HighloadWalletV3 } from './wrappers/HighloadWalletV3';
import { HighloadQueryId } from './wrappers/HighloadQueryId';

const CODE = Cell.fromBoc(Buffer.from('', 'hex'))[0];

async function findSubwalletId() {
    // ВАША ПОЛНАЯ МНЕМОНИКА (24 слова через пробел)
    const mnemonicStr = "energy chase topple spread reform scrap ridge draw hospital girl slush flower spread tired else verify stairs above rely maple lift visa fog grief";
    const mnemonic = mnemonicStr.split(' ');
    
    console.log('Мнемоника:', mnemonic.length, 'слов');
    
    const keyPair = await mnemonicToPrivateKey(mnemonic);
    console.log('Публичный ключ:', keyPair.publicKey.toString('hex'));
    console.log('');
    
    const client = new TonClient({
        endpoint: 'https://toncenter.com/api/v2/jsonRPC',
        apiKey: '799012f0eb0e8cbb554cb76efea226498cfe594ee4c1c2d77c6f5de58b23dd82'
    });
    
    // Ваш реальный адрес кошелька
    const realAddress = "UQBa22xvURSQakGuC-09FRLE3ox50pWpqmuE84yYx_Gop1sV";
    console.log('Реальный адрес кошелька:', realAddress);
    console.log('Ищем правильный subwalletId...\n');
    
    // Проверяем возможные subwalletId
    const possibleIds = [0, 1, 100, 4269, 10000, 12345, 999999];
    
    let found = false;
    
    for (const testId of possibleIds) {
        const wallet = client.open(
            HighloadWalletV3.createFromConfig(
                {
                    publicKey: keyPair.publicKey,
                    subwalletId: testId,
                    timeout: 86400,
                },
                CODE
            )
        );
        
        const generatedAddress = wallet.address.toString();
        const match = generatedAddress === realAddress;
        const indicator = match ? '✅ СОВПАДАЕТ!' : '';
        
        console.log(`subwalletId ${testId}: ${generatedAddress} ${indicator}`);
        
        if (match) {
            found = true;
            console.log(`\n🎉 НАЙДЕНО! Правильный subwalletId = ${testId}`);
            console.log(`Обновите .wallet.json с этим значением`);
        }
    }
    
    if (!found) {
        console.log('\n❌ Не найдено совпадений среди проверенных значений');
        console.log('Возможно, контракт не деплоен или мнемоника не соответствует');
    }
}

findSubwalletId().catch(console.error);